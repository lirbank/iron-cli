...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    for (var i = 0; i < value.packages.add.length; i++) {
      self.installMeteorPackage(value.packages.add[i], {cwd: appDirectory});
    }

    // Remove packages configured to be removed in the master config file
    for (var i = 0; i < value.packages.remove.length; i++) {
      self.removeMeteorPackage(value.packages.remove[i], {cwd: appDirectory});
    }
